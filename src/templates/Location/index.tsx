...
import { Location, LocationProps } from 'services/Location'
...
export function LocationTemplate() {
  ...
  const [rows, setRows] = useState<LocationProps[]>([])
  ...
  useEffect(() => {
    setIsLoading(true)
    debounce(() => {
      Location.getAll(pagination, search).then((response) => {
        setIsLoading(false)
        if (response instanceof Error) {
          setError(true)
          setOpen(true)
  ...
  return (
    <Base>
      <ToolBar
        ...
        textNewCkickButton={() => navigate(`/locations/details/new`)}
      />
      <TableContainer
        component={Paper}
        variant="outlined"
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/locations/details/${row.id}`)}
                  >
                    <EditIcon />
                  </IconButton>
